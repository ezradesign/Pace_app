...
**Ultima actualizacion de este archivo:** 2026-08-04 - sesion 154 (v0.87.0; se retiraron del encabezado la version v0.84.0 y el resumen de la sesion #151, que siguen en `CHANGELOG.md` y en su diario — este archivo no debe crecer)
**Build entregado:** `index.html` **v0.87.0** (hash `C47EA596DBDEB67D`; difiere de HEAD en **exactamente 2 lineas** —`<title>` y `PACE_VERSION`— y **no contiene ni un byte CR**: desde s153 el build normaliza los finales de linea al leer, asi que el artefacto **ya no depende del worktree de quien lo genera**. `PACE_standalone.html` restaurado **byte-identico** — hash `998e3e35...`, decision s134). **OJO**: verificar SIEMPRE el cierre cargando `index.html`, no solo `PACE.html` — el build envuelve cada modulo en un IIFE y hay fallos que solo salen ahi (el `useState` pelado de s144 estuvo DOS versiones publicado). **Desde s150 eso ya no depende de acordarse**: `npm run verify` lo caza por analisis de ambito y devuelve codigo de salida, y **desde s152 comprueba ademas catalogos, i18n, precache y glifos**. **Desde s154 hay ademas `npm run test:e2e`**, que abre un navegador de verdad sobre ese mismo `index.html` y ejecuta el checklist de cierre entero (13 tests, ~25 s) — es el **paso 4** del cierre y va DESPUES de regenerar, para probar el artefacto que se va a commitear. **El CI repite los tres en cada push** (`verify` + frescura + `e2e`), e incluye lo unico que el `verify` NO puede comprobar: que el `index.html` **committeado** sea el build de las fuentes (su aviso de deriva es `[INFO]` a proposito, porque corre justo ANTES de regenerar). `PACE_standalone.html` sigue en v0.71.0 A PROPOSITO (export bajo demanda, s134).
...
| Archivo | Rol | Version |
|---|---|---|
| `PACE.html` | Entry point de desarrollo modular | **v0.87.0** |
| `PACE_standalone.html` | Bundle offline autocontenido — export BAJO DEMANDA (s134), NO se regenera al cerrar | **v0.71.0** |
| `index.html` | Artefacto WEB/PWA canonico (mismo compilado + `<link rel="manifest">`). **Es lo que conduce la suite E2E de s154**, nunca `PACE.html` | **v0.87.0** |
...
## Proxima sesion -- **WRANGLER** (lo ultimo del frente CI), o volver a producto

> **La red de seguridad esta COMPLETA en sus dos mitades**: la ESTATICA (`npm run verify`, 4
> tandas y 32 comprobaciones, ~6 s) y la de COMPORTAMIENTO (`npm run test:e2e`, 13 tests, ~25 s,
> s154). **El CI repite las dos en cada push**, en dos jobs, y anade la frescura del artefacto.
>
