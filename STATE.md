...
**Y despues, FASE 1.6 (ajustes)**, ya definida en el ROADMAP: ocultar sin borrar el estilo de
timer y el circulo «organico» via constante unica + **migrar los valores huerfanos** (sin eso,
quien eligio el analogico queda atrapado) · **bug del boton fantasma** del descanso entre series
(sospechoso: `transition:'all'` animando el `fontWeight` en `TweaksPanel.jsx:290`; MEDIR antes) ·
**idioma «Auto»** que re-evalua en cada arranque reutilizando `detectInitialLang()`.
