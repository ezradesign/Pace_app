...
**Version actual:** v0.117.1 (s186 - **EL ESTANTE DE VIAJES**, y su tarjeta elegida MIRANDOLA. La separacion que pide la Fase 5 («separar Tecnicas de Viajes»), construida: **los viajes viven en su propia lista (`window.BREATHE_VIAJES`) y llegan por su propia prop**, nunca por `BREATHE_ROUTINES`, y de esa estructura salen GRATIS las tres reglas que necesitan -- el filtro «≤ 5 min» no los cuenta, «Para ahora» no los propone, no compiten en el orden. La lista esta **VACIA a proposito** (el contenido CTB queda fuera de v1, s180) y sin viajes no se pinta ni la cabecera. **Se cerro un agujero de premium antes de que existiera**: `canAccessRoutine` es FAIL-OPEN con los ids que no conoce, asi que un viaje de pago se habria abierto gratis. **Y en v0.117.1 la tarjeta deja de ser negra**: gana un lavado del color de Respira con los pulmones del modulo sangrando por la derecha, elegido entre **19 variantes pintadas** sobre la app y con el contraste medido -- el terracota puro se queda en 2,80:1 y los verdes se descartan porque cada modulo tiene su color. El lavado se compone con TOKENS (`--breathe-soft` sobre `--paper-2`), asi que vale en las dos paletas, y **dos tests lo miden en la pagina, uno por paleta**. **200 → 209**, 7 mutantes en rojo y uno declarado que no muerde con su razon.)
