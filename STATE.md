...
- **[FASE 3 · los emisores — PASO 1 CERRADO, falta el emisor]** Plan de trabajo en
  [`docs/HANDOFF_s171.md`](docs/HANDOFF_s171.md). **s170 cerró el prerrequisito**:
  Mueve y Estira ya saben su tiempo activo (`useActiveClock` + `v1TrabajoActivo`,
  4 mutaciones que muerden). Y **las cuatro decisiones que este bloque daba por
  abiertas las contesta `docs/product/EVENTOS_SCHEMA.md`** §6.3/§6.4/§7.1/§7.2 —
  leerlo ANTES de razonar nada. Lo que queda de verdad sin resolver es **el
  desajuste `kind:'body'`**: el registro de Caminos lo usa y `EVENT_STEP_KINDS` no
  lo tiene, así que sin mapeo a `move`/`stretch` **todos** los `path.step.completed`
  de cuerpo se pierden en silencio.
  El plan viejo, superado: [`docs/HANDOFF_s170.md`](docs/HANDOFF_s170.md). La condición de entrada
