...
**Version actual:** v0.94.0 (s163 — **LOS CINCO DE LA REGLA 1, Y DOS PRUEBAS EN VEZ DE UNA MIRADA**. El trinquete de s162 tenia congelados los cinco archivos por encima de 500 lineas y esta version los trocea **todos**: `_responsive.js` **1132 -> 438**, `FocusTimer.jsx` **686 -> 481**, `tokens.css` **676 -> 322**, `TweaksPanel.jsx` **534 -> 467** y `state-core.jsx` **515 -> 449**. `DEUDA_500` queda **VACIA**. **+276 lineas y ni una de codigo**: el cuerpo se movio con scripts que extraen el rango exacto y asertan los bordes. Los dos CSS se **CORTAN por un punto**, porque en CSS el orden es la semantica y `--pace-skin` depende de el. Y se probo dos veces: **huella de reglas identica** (45435 bytes, en archivos y en navegador) y **0 pixeles distintos** contra el artefacto de HEAD servido en paralelo, en dos anchos y dos paletas. Detalle abajo.)
**Version anterior:** v0.93.0 (s162 - **EL TEST INTERMITENTE TENIA RAZON**. Sesion que empieza como AUDITORIA y acaba como saneamiento. La suite llevaba **un rojo cada dos pasadas** en reduced-motion (el aro mide 420 en vez de 406) y no era el instrumento: es el **mecanismo de s160 un nodo mas abajo**. El bloque que hace de horizonte consume `--pace-activities-overlap` como `margin-top` negativo y **no estaba en la exencion de s160** (solo el aro y sus cuatro nodos interiores), asi que bajo el kill de reduced-motion ese margen es una TRANSICION: el alto del stack aterriza en otro frame mientras el motor mide en la misma tarea, el desbordamiento se queda clavado en **11 px** -los de s156, dados por cerrados en s160-, el guard "nunca encoger a ciegas" revierte D a 420 y **gasta su unico reintento**; cuando el reintento corre la misma carrera, el motor **se rinde en 420 y ahi se queda**. El rojo sale en la maquina RAPIDA y desaparece con la CPU frenada. Ademas: **«Regresas» (`first.return`) se perdia por una CARRERA** —el `setTimeout(0)` del rollover puede dispararse antes de que se evalue el modulo de logros, porque el artefacto son **109 scripts** en tareas separadas, y el `try/catch` vacio entierra el ReferenceError—, asi que el hallazgo de s148 era cierto en lo esencial pero no era «nunca»: **lo decide la carga**. Y la regla 1 pasa a estar **vigilada por el `verify`** con un trinquete que se cazo a si mismo. Detalle abajo.)
**Version anterior (2):** v0.92.0 (s161 — **LA PALETA SIGUE AL SISTEMA, SALVO CUANDO EL ARO ES EL SOL**. Nace el modo **Auto** de paleta, que **REVISA** la decision de s89 sin anularla: «el sistema solo manda en el primer arranque» sigue siendo cierto para quien elige a mano; lo que cambia es que ahora se puede elegir **que mande el sistema**. Cuatro decisiones del usuario con su coste medido delante. **Por SISTEMA y no por hora** porque esta app **ya tiene un dia y dura 25 minutos** (`--pace-k` recorre amanecer→mediodia→noche dentro de cada bloque). **Suspender durante un bloque** porque cambiar en vivo cuesta 32 de 66 frames y, sobre todo, en oscuro `--sun-shade` y `--sun-cast` valen **cero** por decision de s158: a mitad de sesion el sol perderia su sombra. El cambio de paleta **no era un corte seco** —1875 declaraciones, **88 fundiendose a cuatro velocidades** y 1787 saltando— y ahora los tokens interpolan solos por `@property`, unica via que alcanza los estilos **inline**; con el Pomodoro parado es **gratis** (66 frames contra 67 del control). Detalle abajo.)
**Ultima sesion:** #163 -- 2026-08-17 - **LOS CINCO DE LA REGLA 1, Y DOS PRUEBAS EN VEZ DE UNA MIRADA**. Bump **v0.93.0 -> v0.94.0**. **(1) LOS CINCO, TROCEADOS, Y LA DEUDA VACIA.** `_responsive.js` **1132 -> 438** (el JS que compone la luz a `_responsive.atmosfera.js`, 396, y las dos pieles @media a `_responsive.pieles.js`, 403) · `FocusTimer.jsx` **686 -> 481** (reparte en sus DOS hermanos, que nacieron para esto en s102 y s124: las curvas de la luz y `focusStyles` a `.support`, la barra, el analogico y su dispatcher a `.parts`) · `tokens.css` **676 -> 322** (el COMPORTAMIENTO a `app/motion.css`, 400) · `TweaksPanel.jsx` **534 -> 467** (el estilo sin UI a `TweaksPanel.support.jsx`, 100) · `state-core.jsx` **515 -> 449** (la paleta a `state-core.palette.jsx`, 107). **3833 lineas antes y 4109 despues: +276 y NI UNA de codigo** — todo lo que crece son cabeceras y punteros. **(2) MOVER BYTES, NO RETECLEAR.** Cada troceo lo hizo un script de un solo uso que extrae el RANGO EXACTO y aserta los dos bordes y lo que debe quedar en cada mitad. Las aserciones pagaron dos veces: el `Object.assign` **no era la ultima linea** (`FocusTimer.support.jsx` acaba en salto y `FocusTimer.jsx` no — de ahi que el guard cuente 686 donde `wc -l` dice 685) y salto **antes de escribir nada**; y la lista de nombres que cruzan de `_responsive.atmosfera.js` a la hoja **se derivo** de las 22 interpolaciones reales del template literal y se exigio que el conjunto fuera exactamente los 13 publicados. **(3) LOS DOS CSS SE CORTAN, NO SE EXTRAEN**, porque en CSS el ORDEN ES LA SEMANTICA: sacar un bloque de en medio deja las mismas reglas en otra cascada. Y hay un contrato que depende de eso: **`--pace-skin` vale `movil` en la hoja base y `escritorio` en el @media de las pieles, misma especificidad**, asi que a >=769px gana la que se inyecta DESPUES; al reves, la home de escritorio se creeria movil y `main.jsx` (s160) renderizaria el orden de lectura de la otra piel. Ademas la hoja **desestructura** `window.paceAtmosfera` en su cuerpo, asi que **el template literal no se toco ni un caracter**, y las pieles no llevan una linea de logica (medido: **0 interpolaciones de 22**). **(4) DOS PRUEBAS, PORQUE LA SUITE NO COMPARA NI UN PIXEL.** (a) **Huella de REGLAS** —comentarios fuera, espacio normalizado— **identica**: 45435 bytes y el mismo sha antes y despues, en archivos (`473c5319…`) y **en navegador** leyendo los 18 `<style>` del documento en orden (`c022e1c9…`). (b) **0 PIXELES DISTINTOS** sirviendo el `index.html` de HEAD **en paralelo**: 0 de 921 600 a 1280x720 y 0 de 329 160 a 390x844, en crema y en oscuro, consola limpia en las cuatro. **Primera vez que este proyecto compara pixeles.** **(5) EL BANCO CAZO UNA MENTIRA DEL INSTRUMENTO**: la primera huella viva leyo un `tokens.css` de 36883 bytes, o sea el ANTERIOR al corte — `npm run verify` compara el build con el disco y luego **RESTAURA** el artefacto, asi que `index.html` seguia siendo el de antes (regenerar es el paso 3 del cierre). **(6) EL CHECKER MENTIA SOBRE SI MISMO** con la deuda vacia: el `NO_CUBRE` decia «los cinco siguen por encima» y el mensaje verde cantaba «0 con deuda registrada», cierto e incomprensible. Corregidos los dos, y **el guard comprobado con la lista vacia** (un archivo de 501 lineas lo pone rojo con «trocear, no anadir a DEUDA_500»). **(7) VERIFICACION TRAS CADA TROCEO, no al final**: `npm run verify` (ambito del compilado + biyeccion + guard) y la suite **67/67**, cinco veces. **(8) LO QUE NO SE CUBRE**: `MoveSessionV1.jsx` esta **exactamente en 500** —el techo, y ahora el verify lo canta como «el mayor»— · `build-standalone.js` sigue en 567 y fuera del alcance, declarado · los pixeles se compararon con el **Pomodoro PARADO** y solo en la home (ni sesion viva, ni Respira, ni Mueve, ni Caminos) · ingles sin mirar · **Respira sin diagnostico por quinta sesion** · el tiron del arco sigue esperando el banco en el telefono. Diario: [session-163](./docs/sessions/session-163-troceo-regla-1.md).
