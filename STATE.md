...
**Version actual:** v0.117.0 (s186 - **EL ESTANTE DE VIAJES**, sobre lo de v0.116.0. La separacion que la Fase 5 pide con estas palabras («separar Tecnicas de Viajes»), construida. **Los viajes viven en su propia lista (`window.BREATHE_VIAJES`) y llegan por su propia prop**, nunca por `BREATHE_ROUTINES`: de esa estructura salen GRATIS las tres reglas que necesitan -- el filtro «≤ 5 min» no los cuenta, «Para ahora» no los propone, no compiten en el orden-- **sin escribir ni una excepcion**. La lista esta **VACIA a proposito** (el contenido CTB esta fuera de v1, s180) y sin viajes no se pinta ni la cabecera. Tarjeta en oscuro, arriba del todo. **Y se cerro un agujero de premium antes de que existiera**: `canAccessRoutine` es FAIL-OPEN con los ids que no conoce, asi que un viaje de pago se habria abierto gratis. **200 → 206**, 5 mutantes en rojo y **uno declarado que no muerde con su razon**.)
