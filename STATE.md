...
**Sesion anterior:** #156 -- 2026-08-04 - **UN NODO OPCIONAL NO PUEDE DECIDIR SI HAY GEOMETRIA**. Bump **v0.88.1 -> v0.89.0**. **(1) EL ORDEN LO IMPUSO EL USUARIO**: medir → entender → proponer → **esperar su eleccion** → implementar. Se entrego diagnostico con 20 puntos y tres direcciones visuales; eligio la **B, «amanecer contenido»**, con la correccion estructural **ANTES** de la atmosfera. Eventos Fase 2A queda **POSPUESTO**: ni un emisor, ni backup, ni `privacy.html`. **(2) LA EVIDENCIA DE PARTIDA NO REPRODUCIA, Y SUS DOS MITADES SE EXCLUYEN.** s149 anoto a 1280×720 un aro de ~406×406 **y** las variables de geometria **vacias**. Medido: **406 es exactamente lo que publica el motor** —los fallbacks dan 360 en Desktop y el clamp de 58vh en movil—, asi que un aro de 406 **prueba** que el motor gobernaba. Ademas **`[data-pace-timer-dial]` no existe en el repositorio** (el real es `[data-pace-dial-fit]`) y **`window.__PACE_HOME_GEOMETRY_VARS__` tampoco**: cero coincidencias en todo `.js/.jsx/.html/.md`. La causa del «vacias» esta **verificada en el codigo**: **ninguna regla CSS declara esas variables**, solo las escribe `home-geometry.js` como estilo **inline sobre `documentElement`**, de modo que una sonda que las buscara en las hojas de estilo encontraria vacio con el motor funcionando. **(3) PERO EL FALLO EXISTIA, EN OTRO ESTADO.** `SuggestedPathCard` retorna null en dos ramas y `getSuggestedPath()` **NUNCA** devuelve null con catalogo no vacio (`state-paths.jsx:169-174` garantiza `catalog[0]`), asi que la unica rama real es **un Camino en curso**. Con esa semilla —forma exacta de `startPath()`, **sin borrar un solo nodo del DOM**— el guard `if (!body || !dial || !spc || !act) return;` apagaba el motor en los **NUEVE** breakpoints. Y **no se recuperaba al salir**: `attach()` corria UNA vez y `ro.observe(spc)` seguia apuntando a un nodo que React ya habia sustituido (medido: `mismoNodoTrasVolver: false`, `nodoViejoConectado: false`); el observador de `body` no dispara porque su tamaño lo fija el flex. Solo despertaba con un resize del usuario o un `pace:home-relayout` **que nadie emite en ese camino**. **(4) EL INVARIANTE QUE ESTE REPO DECLARABA ERA FALSO.** `_responsive.js` afirmaba que recorte y solapamiento salen de una sola fuente y «no pueden desincronizarse nunca»: con el motor apagado, a 390×844, la tarjeta subia **41,7 px** sobre un aro con `clip-path: inset(0px)`, o sea **SIN recortar** — cada consumidor traia su propio fallback (la tarjeta a la estimacion CSS, el recorte a `0px`). En Desktop el fallo era el complementario: solapamiento 0, Actividades sin subir, **circulo cerrado completo**. **(5) DOS DEFECTOS MAS QUE SOLO SALIERON AL MEDIR.** El **arranque tardaba 1345 ms**: DOM listo a **67 ms**, primera publicacion a **1345 ms** y solo **DOS frames** por medio —`attach()` reintentaba por `requestAnimationFrame` y `schedule()` gastaba otro, y en el arranque los frames estan hambrientos—, asi que el aro se pintaba al fallback y **saltaba** 360→406 en cada carga. Y uno **PREVIO Y PUBLICADO**: con `prefers-reduced-motion: reduce` el aro salia a **244 px** en vez de 406, confirmado **sirviendo el `index.html` de HEAD en paralelo**. Instrumentando el bucle se vio la mecanica: el alto medido se quedaba **clavado** mientras D bajaba de 420 a 322, o sea que **agotaba sus ocho pasadas restando A CIEGAS**. **(6) LO IMPLEMENTADO.** Motor: guard reducido a **home + aro** (tarjeta y Actividades **opcionales**) · **observadores en dos fases** —uno espera al montaje y **se desconecta**, otro vigila el `childList` **DIRECTO** del stack (sin `subtree`, sin `attributes`, sin `characterData`) y re-suscribe el `ResizeObserver` a los nodos vivos; el aro **nunca** se observa, eso si seria un bucle— · **primera pasada SINCRONA** (publica a los 164 ms, con la home) · **nunca encoger a ciegas**, con **un** reintento por episodio. Fallback UNICO: `--pace-dial-d` y `--pace-horizon` resuelven motor-o-CSS **en un solo sitio**, desaparece el `360px` a mano, y recorte y solapamiento consumen **el MISMO token** ⇒ la frase del comentario pasa a ser cierta **por construccion**. **(7) AMANECER.** Halo en `[data-pace-dial-fit]::before` recortado por el **MISMO** `clip-path` que corta el aro ⇒ la luz emerge **de detras del horizonte**; **REUTILIZA** `paceGlowRamp` y `paceGrainUrl` de `SessionShell.jsx` (s140) horneando su salida en la hoja, asi que no se duplica ni la curva ni el grano antibanding, con fallback donde no exista `color-mix`. Linea de alba anclada a `--pace-horizon`. Tokens **`--dawn-soft`/`--dawn-line` en las dos paletas**, con tono propio: no se reutilizan `--breathe` ni `--move`, que son colores de MODULO. **Tres estados por atributo estable** —reposo, `data-pace-dial-running` y `data-pace-dial-paused` (nuevo, declarativo, **sin tocar logica del temporizador**)— que solo mueven **intensidad**; el estado **nunca** se comunica solo con eso. **(8) RITMO MOVIL.** Techo por ancho **0.86 → 0.92**, derivado del ancho **REALMENTE usable** (W−24 px por el padding lateral), no de gusto; y `--pace-home-slack` reparte el sobrante **38/62** en vez de centrarlo. A 390×844 el aro pasa de **335 a 359** y el aire muerto de arriba de **91 a 62 px**; `dial/alto` sube de 0.39-0.40 a **0.414-0.425**. La guia de 0.44-0.48 **no es alcanzable** a 390 px de ancho: el aro topa por ANCHO y 0.44 exigiria ~371 px sobre 366 usables. **(9) 14 PRUEBAS NUEVAS Y 12 ROJOS QUE MORDIERON.** Casi todas relacionales; la de «el contador no despierta al observador» se prueba **sin instrumentar el codigo**, instalando un `MutationObserver` con la **misma configuracion** sobre la **misma raiz**. **12/12 rojos** con `index.html` restaurado byte a byte y hash comprobado (`9D5F6F8426ABB66A`), exigiendo la cadena **exactamente una vez** (s155) y sin `shell:true` (s154). **Declarado sin rojo**: «controles sin duplicados y alcanzables», que no admitia un sabotaje minimo de una sola cadena. **(10) EL INSTRUMENTO MINTIO CINCO VECES.** El spec leia `null` donde el banco media 406 porque **el artefacto no se habia regenerado** · **backticks dentro del template literal** de `_responsive.js` abortaron el build (trampa de s139, otra vez) · el horizonte se eligio **por selector** en vez de por posicion, y en Desktop el `order` pone Actividades ahi · se midio **durante la animacion de entrada**, que desplaza `main-content` **10 px**, y las diferencias salian de 10 px **exactos** ⇒ casi se reporta una desincronizacion inexistente · y **`test.use({ reducedMotion })` no llego a aplicarse**, lo que cazo un **guard** que asertaba el media query antes de medir: sin el, la prueba habria pasado **en verde midiendo un navegador sin reduced-motion**. **(11) DEUDA DE A11Y DOCUMENTADA, NO ARREGLADA.** **Orden DOM ≠ orden visual en escritorio** (WCAG 2.4.3, **previo**): visual `dial > act > spc`, DOM `dial > spc > act`, asi que el foco de teclado salta del aro a la tarjeta del fondo y **despues** sube a Actividades. **No se consagra en ningun aserto** — la prueba comprueba el orden **VISUAL**, y la de controles solo que no haya duplicados y que todo acepte el foco. Propuesta para la sesion de ContinuityCard: renderizar el orden canonico **en el DOM** por piel, con el mismo breakpoint de 769 px que ya usa el motor. **(12) DEUDA MECANICA EN COMMIT APARTE**: `tests/eventos.spec.js` estaba en **502 lineas** y se partio en tres (46 · 273 · 216) sin tocar una linea de cuerpo — **39 verdes antes y despues**. **(13) VERIFICACION**: `npm run verify` **PASA en 5,3 s** con v0.89.0 coherente en los 3 sitios · **`npm run test:e2e` PASA 39/39** contra el artefacto recien regenerado (25 previas + 14 nuevas, **sin una regresion**) · `index.html` **0 bytes CR** de 1 370 239 · `PACE_standalone.html` restaurado **`998E3E358D689036`** · navegador real con **SW y caches purgados** en 9 breakpoints × ES/EN/claro/oscuro: **cero errores de consola**, cero desborde horizontal y **escritorio sin una regresion** (406/65 · 456/73 · 487/78 · 520/83). **(14) LO QUE NO SE CUBRE**: ni un pixel comparado; la microcausa del congelamiento de la medida bajo reduced-motion queda **sin identificar** y a 1280×720 la home queda ahi con **11 px de scroll** en vez de encajar exacta. Diario: [session-156](./docs/sessions/session-156-home-amanecer.md).
...
| Archivo | Rol | Version |
|---|---|---|
| `PACE.html` | Entry point de desarrollo modular | **v0.90.0** |
| `PACE_standalone.html` | Bundle offline autocontenido — export BAJO DEMANDA (s134), NO se regenera al cerrar | **v0.71.0** |
| `index.html` | Artefacto WEB/PWA canonico (mismo compilado + `<link rel="manifest">`). **Es lo que conduce la suite E2E de s154**, nunca `PACE.html` | **v0.91.0** |
...
## Proxima sesion -- **cerrar el tiron del arco** (espera dato del usuario), luego DIAGNOSTICO DE RESPIRA, luego FASE 2 de eventos

> **Lo primero que hay encima de la mesa** es el resultado del banco de cuatro aros en el telefono
> del usuario (ver «El tiron del arco» en Pendiente): sin ese dato no se toca el Pomodoro. Y
> **queda sin abrir el punto 4 del encargo de s160**: el **diagnostico de Respira** —leer el
> codigo real, identificar que representa cada barra del progreso inferior, documentar las
> redundancias (fase, respiracion, ronda y total pueden estar diciendo lo mismo tres veces) y
> traer un menu de decisiones—, **sin implementar nada**.
>
> **La red de seguridad esta COMPLETA en sus dos mitades**: la ESTATICA (`npm run verify`, 4
> tandas, ~8 s) y la de COMPORTAMIENTO (`npm run test:e2e`, **58 tests, ~60-80 s**). **El CI repite
