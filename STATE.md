...
**Ultima actualizacion de este archivo:** 2026-08-04 - sesion 156 (v0.89.0; se retiro del encabezado el resumen de la sesion #153, que sigue en `CHANGELOG.md` y en su diario — este archivo no debe crecer)
**Build entregado:** `index.html` **v0.89.0** (hash `C86AD7128E633A65`; **no contiene ni un byte CR** (0 de 1 370 239): desde s153 el build normaliza los finales de linea al leer, asi que el artefacto **ya no depende del worktree de quien lo genera**. `PACE_standalone.html` restaurado **byte-identico** — hash `998e3e35...`, decision s134). **OJO**: verificar SIEMPRE el cierre cargando `index.html`, no solo `PACE.html` — el build envuelve cada modulo en un IIFE y hay fallos que solo salen ahi (el `useState` pelado de s144 estuvo DOS versiones publicado). **Desde s150 eso ya no depende de acordarse**: `npm run verify` lo caza por analisis de ambito y devuelve codigo de salida, y **desde s152 comprueba ademas catalogos, i18n, precache y glifos**. **Desde s154 hay ademas `npm run test:e2e`**, que abre un navegador de verdad sobre ese mismo `index.html` y ejecuta el checklist de cierre entero — es el **paso 4** del cierre y va DESPUES de regenerar, para probar el artefacto que se va a commitear; s155 la subio a 25 tests con `pace.events.v1` (dos de ellas conduciendo **dos pestanas de verdad**) y **s156 la deja en 39 (~29 s)** con las 14 de la geometria de la home. **El CI repite los tres en cada push** (`verify` + frescura + `e2e`), e incluye lo unico que el `verify` NO puede comprobar: que el `index.html` **committeado** sea el build de las fuentes (su aviso de deriva es `[INFO]` a proposito, porque corre justo ANTES de regenerar). `PACE_standalone.html` sigue en v0.71.0 A PROPOSITO (export bajo demanda, s134).
