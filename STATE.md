...
**Ultima actualizacion de este archivo:** 2026-08-03 - sesion 149 (v0.82.0; se retiraron del encabezado la version v0.79.1 y el resumen de la sesion #142, que siguen en `CHANGELOG.md` y en sus diarios — este archivo no debe crecer)
**Build entregado:** `index.html` **v0.82.0** (regenerado en s149; `PACE_standalone.html` restaurado **byte-identico** — hash `998e3e35...` antes y despues, decision s134). **OJO**: verificar SIEMPRE el cierre cargando `index.html`, no solo `PACE.html` — el build envuelve cada modulo en un IIFE y hay fallos que solo salen ahi (el `useState` pelado de s144 estuvo DOS versiones publicado). `PACE_standalone.html` sigue en v0.71.0 A PROPOSITO (export bajo demanda, s134).
...
| Archivo | Rol | Version |
|---|---|---|
| `PACE.html` | Entry point de desarrollo modular | **v0.82.0** |
| `PACE_standalone.html` | Bundle offline autocontenido — export BAJO DEMANDA (s134), NO se regenera al cerrar | **v0.71.0** |
| `index.html` | Artefacto WEB/PWA canonico (mismo compilado + `<link rel="manifest">`) | **v0.82.0** |
...
| `app/state-core.jsx` | Store, `loadState`, tema y toast. El rollover y las migraciones **salieron en s148** | **v0.82.0** |
...
| `sw.js` | Service Worker PWA. **s149: el export offline SALE del precache** (congelado a proposito en v0.71.0 y servido cache-first para siempre). **86 filas de `PRECACHE`**; si se toca, contar filas contra las entradas reales de la cache — `addAll` es atomico | **v0.82.0** |
