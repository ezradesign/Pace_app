...
| Archivo | Rol | Version |
|---|---|---|
| `PACE.html` | Entry point de desarrollo modular | **v0.74.0** |
| `PACE_standalone.html` | Bundle offline autocontenido — export BAJO DEMANDA (s134), NO se regenera al cerrar | **v0.71.0** |
| `index.html` | Artefacto WEB/PWA canonico (mismo compilado + `<link rel="manifest">`) | **v0.74.0** |
...
| `app/state-core.jsx` | Store, loadState, rollover, migraciones, toast | **v0.74.0** |
...
| `sw.js` | Service Worker PWA | **v0.74.0** |
...
## Proxima sesion -- FASE 2, OLA B: los 20 glifos que faltan

> Orden de trabajo vigente: seccion «Camino a v1.0» de [`ROADMAP.md`](./ROADMAP.md) (15 fases).
> **FASE 1 cerrada** · **1.5 cerrada en s138** · **1.6 cerrada en s139** · **cola de banding
> cerrada en s140** · **FASE 2 EN CURSO desde s141**: auditoria hecha y ola A entregada.

### OLA B — dibujar los 20 glifos que no existen

Es la que **quita el `DefaultGlyph` de la vista**, y la que el usuario itera: los glifos que
dibuje o apruebe se portan **LITERALES** (regla s84). La lista exacta, con su zona corporal y su
rutina, esta en la matriz §19.2 de
[audit-mueve-estira-v0.73.1](./docs/audits/audit-mueve-estira-v0.73.1.md) (ordenada con los sin
glifo primero, para que se lea de un vistazo).

**Los 20**: Apretar gluteos · Circulos de hombro · Gato-camello · Hang activo (hoy **Suspension
activa**) · Hollow hold (hoy **Hueco abdominal**) · Isquio a una pierna · Onda espinal · Palmas al
suelo · Pica en escritorio · Plancha · Plancha lateral · Pliegue adelante · Puente toracico ·
Rana · Rezo invertido · Rodar hacia abajo · Sentadilla a silla · Sentadilla bulgara · Superman ·
Couch stretch (hoy **Cuadriceps en pared**).
...
- **Ola C — renombrar los ~30 nombres con ingles que SI tienen glifo.** Exige el cambio coordinado
  `name` + clave de glifo + i18n (s108) **mas su entrada en `VISUAL_ALIAS`**, que desde s141 es
  parte del contrato: sin ella, las rutinas propias ya guardadas se quedan sin glifo en silencio.
- **Ola D — segundo nivel visual (§19.3)**: diagrama de ejecucion en el runner. Lo respalda una
  medida de s141: **41 de 47 glifos son una sola postura estatica sin direccion**, asi que el
  glifo de 44x44 no puede explicar la tecnica por mucho que se redibuje.
- **Ola E — consumir `level`/`intensity`** (declarados en 22 de 28 rutinas, sin ningun consumidor)
  y separar intensidad de nivel tecnico (§29.2).

### Decisiones sueltas que la auditoria dejo sobre la mesa

- **`Superman`**: se mantuvo en la ola A a proposito (nombre propio legible en espanol). Si el
  usuario lo quiere en espanol, ojo con «Extension boca abajo»: choca con `Apertura de pecho`, de
  la misma rutina.
...
- **`Nordics`** es un glifo que **ningun paso usa**: o falta el ejercicio, o sobra el dibujo.
...
**Restricciones vivas al tocar este modulo**: `MoveSessionV1.jsx` esta **exactamente en 500 ln**,
el tope de CLAUDE.md ⇒ cualquier anadido va a `MoveSessionV1.support.jsx`; y `ExtraModule.jsx`
esta en 454, asi que al retomar Estira **se trocean los datos ANTES** de tocar nada.
