...
**Ultima actualizacion de este archivo:** 2026-08-03 - sesion 151 (v0.84.0; se retiraron del encabezado la version v0.81.0 y el resumen de la sesion #147, que siguen en `CHANGELOG.md` y en sus diarios — este archivo no debe crecer)
**Build entregado:** `index.html` **v0.84.0** (regenerado en s151; el `verify` confirma que el del disco **es** el build de las fuentes — hash `923DA70C0C358B6F`. `PACE_standalone.html` restaurado **byte-identico** — hash `998e3e35...`, decision s134). **OJO**: verificar SIEMPRE el cierre cargando `index.html`, no solo `PACE.html` — el build envuelve cada modulo en un IIFE y hay fallos que solo salen ahi (el `useState` pelado de s144 estuvo DOS versiones publicado). **Desde s150 eso ya no depende de acordarse**: `npm run verify` lo caza por analisis de ambito y devuelve codigo de salida. `PACE_standalone.html` sigue en v0.71.0 A PROPOSITO (export bajo demanda, s134).
...
| Archivo | Rol | Version |
|---|---|---|
| `PACE.html` | Entry point de desarrollo modular | **v0.84.0** |
| `PACE_standalone.html` | Bundle offline autocontenido — export BAJO DEMANDA (s134), NO se regenera al cerrar | **v0.71.0** |
| `index.html` | Artefacto WEB/PWA canonico (mismo compilado + `<link rel="manifest">`) | **v0.84.0** |
...
| `app/state-core.jsx` | Store, `loadState`, tema y toast. El rollover y las migraciones **salieron en s148** | **v0.84.0** |
...
| `sw.js` | Service Worker PWA. **s149: el export offline SALE del precache** (congelado a proposito en v0.71.0 y servido cache-first para siempre). **86 filas de `PRECACHE`**; si se toca, contar filas contra las entradas reales de la cache — `addAll` es atomico | **v0.84.0** |
...
## Proxima sesion -- **RED DE SEGURIDAD: SEGUNDA TANDA** (lo que D5 aparco)

> **Los frentes A y B de la auditoria estan CERRADOS** (s150 y s151). El siguiente paso natural
> es la **segunda tanda de la red de seguridad**: **integridad de catalogos, i18n, precache y
> glifos** — exactamente lo que **D5 dejo fuera a proposito** del `verify` v1.
>
> **No hay que inventar umbrales**: el material ya esta medido. Recuento i18n **clave a clave
> contra HEAD** (s148 midio 195 ES / 195 EN en su split; s151 lo hizo con el diff y salio
> biyectivo) · **filas de `PRECACHE` = entradas reales de la cache** (s149; `addAll` es atomico y
> una sola 404 tira el install entero) · **47 glifos de ejercicio** · **58 mascaras de logro**. El
> patron esta fijado por s150: cada comprobacion es **un aserto con su numero esperado**, no una
> impresion — y **lo que se anada se declara** en la lista de huecos que el script imprime en cada
> pasada, tambien en verde.
>
> **Detras de eso, y no antes, va el CI**: GitHub Actions/YAML, Wrangler, Playwright y proteger
> `main`. No existe `.github/` (verificado en s149). El YAML tiene que invocar algo que ya
> funcione en local.
>
