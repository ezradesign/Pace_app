...
**Version actual:** v0.92.0 (s161 — **LA PALETA SIGUE AL SISTEMA, SALVO CUANDO EL ARO ES EL SOL**. Nace el modo **Auto** de paleta, que **REVISA** la decision de s89 sin anularla: «el sistema solo manda en el primer arranque» sigue siendo cierto para quien elige a mano; lo que cambia es que ahora se puede elegir **que mande el sistema**. Cuatro decisiones del usuario con su coste medido delante. **Por SISTEMA y no por hora** porque esta app **ya tiene un dia y dura 25 minutos** (`--pace-k` recorre amanecer→mediodia→noche dentro de cada bloque). **Suspender durante un bloque** porque cambiar en vivo cuesta 32 de 66 frames y, sobre todo, en oscuro `--sun-shade` y `--sun-cast` valen **cero** por decision de s158: a mitad de sesion el sol perderia su sombra. El cambio de paleta **no era un corte seco** —1875 declaraciones, **88 fundiendose a cuatro velocidades** y 1787 saltando— y ahora los tokens interpolan solos por `@property`, unica via que alcanza los estilos **inline**; con el Pomodoro parado es **gratis** (66 frames contra 67 del control). Detalle abajo.)
**Version anterior:** v0.91.0 (s160 — **UNA TRANSICION DE 0,01 ms SIGUE SIENDO UNA TRANSICION**. Dos deudas de s156 cerradas y **un tirón que NO reproduce**. La microcausa de reduced-motion llevaba una sesión sin identificar y es de manual: el kill de `tokens.css` pone `transition-duration` en 0,01 ms sobre TODO y, como el valor **inicial** de `transition-property` es **`all`**, cualquier cambio de geometría **pasa a ser una transición** — cuyo valor aterriza en otro frame, mientras `applyD()` mide en la MISMA tarea. Se demostró porque **ni un `height !important` en línea movía el aro**: en la cascada solo una transición viva gana a `!important`. Y el orden de foco de escritorio (WCAG 2.4.3, previo) sale del `order` del CSS sobre un DOM quieto: **medido con Tab real**, el foco bajaba a 622 y 698 y **subía** a 496. Detalle abajo.)
**Version anterior (2):** v0.90.0 (s159 — **LA LUZ DEL POMODORO**. Cierra el frente que **s157 y s158 dejaron en el arbol sin commit y sin una linea de documentacion**: este commit publica las tres. El bloqueo lo puso el usuario: **el Pomodoro no estaba centrado en movil**, y medido contra HEAD servido en paralelo el defecto era **PREVIO y estaba publicado** (+11,80 px a 320 y +12,09 a 390 en los dos artefactos) — la atmosfera no lo introdujo, lo hizo VISIBLE porque el halo cuelga del aro. Ademas: el maximo perceptual **se retima a la mitad del bloque**, el enfriamiento pasa a continuo **sin repunte en p=1**, el parpadeo cae de **47 saltos ≥2 % a uno**, la cola recupera el reflejo bajo los chips y **el arco enterrado la tiñe**. Tres defectos mas que encontro el usuario mirando, los tres del mismo tipo: transiciones que se pisan. Detalle abajo.)
**Ultima sesion:** #161 -- 2026-08-08 - **LA PALETA SIGUE AL SISTEMA, SALVO CUANDO EL ARO ES EL SOL**. Bump **v0.91.0 -> v0.92.0**. **(1) EL CI LLEVABA DOS VERSIONES EN ROJO** (#7 v0.90.0 y #8 v0.91.0) mientras s159 y s160 cerraron declarando verde: lo verde era **local**. Localizado sin adivinar —el repositorio es **publico** y la API de *jobs* responde sin auth—: **`verify` VERDE y `e2e` ROJO** en los dos. Reproducido el job `verify` paso a paso aqui (pasa, y el `index.html` committeado coincide byte a byte con el build). **Tres hipotesis descartadas MIDIENDO**: no son los 2 workers (`CI=true` en local da **58/58 en 39,1 s**), no son las mayusculas de rutas (**193 comprobadas** contra el disco leyendo el directorio, porque en Windows `existsSync` miente: **cero desajustes**) y no es el lockfile. Es **Linux**. Los *logs* daban **403 sin auth**, asi que se instalo `gh` (2.97.0) y **el usuario se autentico** — y el log dio la causa en un comando: **2 tests de `home-luz`**, los dos por lo mismo. **`--pace-on` NO ES UN BOOLEANO**: es el interruptor de la luz y **se funde en 1,6 s** (s159). Los dos lo leian **justo tras aparecer `data-pace-dial-running`**, donde vale **0 EXACTO** (curva medida: 0,0039 a los 50 ms · 0,028 a los 100 · 0,373 a los 300 · 0,979 a los 800), asi que el resultado dependia de si el viaje de ida y vuelta de Playwright dejaba pasar un frame: **en local casi siempre, en el runner de Linux no**. Dos versiones en rojo por muestrear un fundido en su primer instante. Se **espera a que encienda** con `expect.poll` en vez de bajar el liston —el contrato asertado sigue siendo «arrancar un bloque enciende la luz»— y **los dos siguen mordiendo**, comprobado saboteando `publicarLuz` para que `--pace-on` valga siempre 0. **Cae ademas una suposicion heredada**: s159 anoto que `page.clock` congela ese fundido, y medida la curva con y sin reloj virtual **es identica**. **(2) LAS CUATRO DECISIONES, cada una con su coste delante.** **(a) Por SISTEMA y no por hora**, y no es preferencia: esta app **ya tiene un dia y dura 25 minutos** —`--pace-k` recorre amanecer→mediodia→atardecer→noche dentro de cada bloque—, asi que una paleta por reloj meteria un **segundo ciclo a otra velocidad**; ademas el SO ya trae programacion horaria, asi que seguirlo **incluye** la hora sin que PACE decida umbrales. **(b) Suspender durante un bloque**, con el disparador en `haySesion -> false` y no «al completar», porque abandonar o resetear tambien terminan un bloque. **(c) Tercera pill «Auto»** como el idioma; elegir a mano la apaga. **(d) Fundido de tokens** por `@property`. **(3) EL CAMBIO DE PALETA NO ERA UN CORTE SECO.** 28 tokens cambian de valor (**no ~40**) y eso mueve **1875 declaraciones computadas** sobre 87 nodos: **88 se funden y 1787 saltan** (94 %), y las 88 a **cuatro velocidades a la vez** (180/200/220/320 ms). No era un corte: era un borron. Medido con `getAnimations()` —lo unico que distingue «salta» de «se funde»— y **control positivo** (una cobaya de 900 ms que tiene que aparecer, y aparece). **(4) COSTE DE LAS CUATRO ALTERNATIVAS, con control negativo** (misma ventana, sin cambiar de paleta): con el Pomodoro **parado** el fundido de tokens es **GRATIS** (66 frames contra 67 del control, cero frames largos); **con sesion viva cuesta hasta el corte seco** (52 de 66), y el coste no lo trae la transicion sino invalidar los `--sun-*` con el halo encendido. **View Transitions descartada** (12 frames largos parado, 14 con sesion, peor 116,8 ms, y **sigue pagando el snapshot bajo reduced-motion** aunque su animacion este muerta); **velo descartado** porque solo funde el PAPEL y deja saltando tinta, bordes y sellos. Registrar los tokens y **no cambiar nada** cuesta **cero**. **(5) LA MEDIDA QUE DECIDIO (b)**: a la misma hora las dos paletas no son el mismo sol con otro papel — `--sun-noon-core` va de alfa 0,70 a 0,30 y **`--sun-shade` y `--sun-cast` valen exactamente CERO en oscuro** (decision de s158, con su porque escrito). A mitad de bloque el sol **perderia su sombra y su proyeccion**. **(6) DOS COMPROBACIONES ANTES DE IMPLEMENTAR, no despues**: declarar una transicion en `:root` **no toca el motor de geometria** (aro 406, `--pace-dial-d` 406px, `--pace-horizon` 65px con y sin la hoja, con y sin reduce) con **control positivo que reproduce la firma exacta de s160** (420 px); y con el Pomodoro parado **no hay sol que fundir** (`--pace-on` 0, halo `none`, grid sin filtro ni sombra) ⇒ los 11 `--sun-*` fuera de la lista. **(7) EL DEFECTO DE s159 OTRA VEZ, cazado antes de publicar.** El `transition` de 320 ms del `body` **perseguia** al token de 640 —una transicion cuyo destino cambia cada frame **se reinicia cada frame**— y dejaba **188 unidades RGB** entre el papel del body y el token, con las tarjetas (estilo **inline**, o sea siguiendo al token exacto) ya oscuras. Contra una **verdad de campo** con todos los seguidores muertos: **237 -> 15** de desviacion, y el control —la misma regla sin marcar el flag— vuelve exactamente a los numeros de antes, asi que lo que arregla es el mecanismo. **No baja a cero a proposito**: `data-pace-essential` queda fuera (WCAG 2.3.3). **(8) REGISTRAR `--breathe` APAGABA LA ATMOSFERA DEL POMODORO.** Lo cazo la suite y se **confirmo que era regresion propia** sirviendo el `index.html` de HEAD. Un token registrado con `@property` computa `rgb(201, 122, 93)` en vez de `#C97A5D`, y `interpolateRingColor` (`TimerDial.jsx:24-33`) le aplica un `hexToRgb` que hace `slice()` sobre dos digitos por canal ⇒ **NaN** ⇒ `--pace-arco` invalido ⇒ **el bloom entero a `background-image: none`**. Aislado bisecando pieza, familia y token. **Auditados los 15**: solo `--breathe` y `--focus` tienen lector en JS en todo `app/` ⇒ **se arreglo el LECTOR en vez de dejarlos fuera** (decision del usuario en la misma sesion): `aRgb` acepta hex y forma canonica y **devuelve un respaldo en vez de NaN**, asi que entran los quince y el modo de fallo pasa de «se apaga media home en silencio» a «el arco usa su color por defecto». `--move` y `--extra` siguen fuera a proposito. **Aserto nuevo que lo consagra de frente** (antes solo se cazaba por rebote, en una prueba de la luz), rojo comprobado revirtiendo el arreglo: muerde con «el arco no es un color en crema». **(9) EL GUARD DE ARRANQUE, Y SU CARRERA.** Sin guard, `PACE.html` da **15 cruces en el arranque** (el primero a 1377 ms sobre `--ink-3`) y con el 0 — demostrado con **control fiel** (servir el mismo artefacto con el selector del guard quitado) tras dos intentos que no valian. Pero el guard **tal como se escribio primero no era fiable**: fallaba solo con ocho workers, con los **trece** tokens compartiendo `startTime`. `data-palette` **no cambia nunca** despues del arranque (comprobado con `MutationObserver` sobre `<html>`), asi que era una **carrera**: si el navegador no habia recalculado el estilo entre el `setAttribute` y el frame siguiente —bajo carga **no lo hace**—, el estilo previo seguia siendo el CLARO y armar la transicion aterrizaba el papel oscuro en el mismo recalculo. Se arregla **leyendo `--paper` antes de armar la transicion**, una linea que parece inutil; y **la sonda que buscaba el defecto leia estilo en cada tick, o sea que forzaba ese recalculo y hacia desaparecer justo lo que iba a medir**. **(10) GUARD DE AUTO EN `secret.dark.mode`**, gemelo del de `secret.bilingual` (s139): el logro premia haber **ELEGIDO** el oscuro, y sin el bastaria con tener el SO en oscuro siete dias para regalarlo. **(11) LA SUITE 58 -> 65, y tres rojos que no eran del producto**: ruido del instrumento (la sonda contaba `--pace-luz` y `--pace-nucleo`, tambien registrados desde s159: **1599 «cruces»** con ocho workers y 0 aislada) · un cruce **REAL** que era la funcion trabajando (con Auto, `emulateMedia` **corre contra la navegacion** y la prueba se sembraba su propio defecto) · y el **guard de cero mordiendo con razon** (8 frames en vez de >10) ⇒ se corrigio la **ventana**, no la tolerancia, y entro un **segundo guard en otro eje**. **(12) EL PUNTO 3 DEL ENCARGO, HECHO**: los enlaces `[abajo]` rotos del CHANGELOG pasan de **12 a 0**, once a su diario y uno retirado (**s58 nunca escribio diario**); ademas cuatro enlaces preexistentes apuntaban a nombres de archivo equivocados y se resuelven **por numero de sesion**. **(13) EL CI, VERDE Y OBSERVADO.** Empujado el commit, el **run #9 cierra en SUCCESS los dos jobs** — y esta vez el conteo **se lee en el log**, no se infiere (la deuda que s154 dejo anotada): **`65 passed (56.0s)`**, mas el paso propio del `verify` diciendo «index.html coincide byte a byte con el build de las fuentes». El unico paso no-success del job `e2e` es `Guardar el informe si fallo: skipped`, que es su `if: failure()`. **(14) VERIFICACION LOCAL**: `npm run verify` **PASA** con v0.92.0 coherente en los 3 sitios (el **CENSO** de i18n pidio subir 510 -> 511 a mano, que es exactamente para lo que existe: la paridad ES/EN es **relacional** y paso, 511 = 511 biyectiva) · **`npm run test:e2e` PASA 65/65** contra el artefacto recien regenerado · **banco de rojos 9/9** —6 de la paleta, 1 del arco y **2 de los asertos reparados del CI**—, cada uno corriendo **exactamente 1 test** y con el artefacto restaurado **byte a byte**; su primera version **mintio dos veces** (mutaciones de CSS buscando la regla minificada, y `-g` partiendose con `shell:true` — «1 failed · 24 passed» en un archivo de **seis** tests). **(15) LO QUE NO SE CUBRE**: el **diagnostico de Respira (punto 2 del encargo) NO se abrio** · **ni un pixel comparado** y **movil sin medir** · quedan **26 filas del CHANGELOG** enlazando a diarios que **nunca se escribieron** (sesiones 1-26), hueco **preexistente**. Diario: [session-161](./docs/sessions/session-161-paleta-automatica.md).

**Sesion anterior:** #160 -- 2026-08-07 - **UNA TRANSICION DE 0,01 ms SIGUE SIENDO UNA TRANSICION**. Bump **v0.90.0 -> v0.91.0**. **(1) EL TIRON DEL ARCO NO REPRODUCE, y las dos hipotesis caen MEDIDAS.** El usuario reporto que el aro «da un tick por segundo». Banco con los dos artefactos servidos a la vez y **alternados A-B-A-B en la misma pasada**, muestreando por rAF el valor computado, el punto guia y las **transiciones vivas** (Web Animations API, que es lo unico que distingue «termina antes» de «la reinician»): la transicion **cubre el segundo entero** —**0 ms quieto** en todos los tramos, 57-60 frames de movimiento— **igual en v0.90.0 que en v0.89.0**. La sospecha de las ~197 publicaciones de s159 cae **por ritmo** (2 en 8 s; el guard `if (valor !== actual)` ya las filtra) **y por control positivo**: forzando una publicacion **en cada frame**, 460x el ritmo real, la transicion **sigue corriendo** (duty 73 %). Comprobado ademas en **PIXELES** —fotografiando el aro cada ~130 ms cambian pixeles en TODAS las tomas; lo unico que cambia una vez por segundo son los digitos—. **El instrumento SI ve el defecto**: con `prefers-reduced-motion` da **1 frame y 983 ms quieto por segundo**, que es la firma exacta del sintoma y la produce `tokens.css:374` (s89, PREVIO) — pero esa via esta **descartada para el usuario**: su Windows tiene las animaciones activadas y en su movil **la luz se funde**. **(2) SUS CONDICIONES, que el banco no podia inventar**: lo vio en **paceweb.pages.dev** (artefacto **byte a byte identico** al local), en un **Doogee Blade 20 Max con Brave** (Chromium, el mismo motor medido), con un bloque de **1 minuto** —25x mas avance por segundo— y **lo que salta es el punto brillante, no el trazo**. A 1 min y en local sigue fluido (55-59 frames/s tambien el punto), y bajo presion de CPU los dos canales caen **JUNTOS** (a 20x: 18,6 fps y hasta **563 ms parado**). **Queda ABIERTO**: se le entrego un banco que corre **en su propio telefono** con cuatro aros (tal cual · sin la mascara de s158 · punto por CSS · **control sin transicion que TIENE que dar tirones**), calibrado en las dos direcciones — y la verificacion **cazo un fallo del propio banco**: sin copiar el kill de `tokens.css` daba «fluido» con menos-movimiento activado, un falso negativo justo en el caso mas probable. **(3) REDUCED-MOTION: LA MICROCAUSA, Y ERA UN FRAME.** Deuda de s156 (aro 420 en vez de 406 y 11 px de scroll a 1280x720). Barriendo D a mano salio algo mas fuerte que lo anotado: **no es el stack el que no responde, es el ARO** —con reduce se queda en 420 con CUALQUIER valor— y siguiendo la cadena, **ni un `height: 340px !important` en linea lo mueve**. En la cascada solo gana a `!important` **una transicion viva**, y `dial.getAnimations()` devolvia **`height:running`**: el kill pone la duracion en 0,01 ms sobre todo y `transition-property` vale **`all`** por defecto, asi que cada cambio de geometria es una transicion y su valor aterriza **dos frames** despues, mientras el motor mide en la misma tarea. Simulado: sincrono, `over` **clavado en 11** mientras D bajaba de 420 a **322** (los numeros exactos de s156); esperando un frame, converge en **dos pasadas** a 406. **Arreglo validado ANTES de proponerlo**: `transition-property: none` en el aro **y en sus cuatro nodos interiores** —`applyD()` mide donde acaba el CICLO **dentro** del aro y esos nodos llevan margenes derivados de D: de ahi los **3 px** que quedaban con el solapamiento en -61 en vez de -65—. Resultado: aro **406** y solape **-65** con y sin reduced-motion, scroll **-1** en los dos. **(4) EL ORDEN DE LECTURA DE LA HOME.** El `order: 1 / 2` del CSS reordenaba con el DOM quieto ⇒ en escritorio el foco iba de «Empezar foco» (387) a «Iniciar camino» (**622**) y «Ver caminos» (**698**) y **SUBIA** a los chips (**496**): WCAG 2.4.3, **medido con Tab de verdad**, previo. En movil no pasaba. Ahora cada piel renderiza su **orden canonico en el DOM** y no queda ningun `order`; la piel se lee de **`--pace-skin`**, que publica la **propia hoja** de `_responsive.js`, para no escribir una tercera copia del breakpoint; y los tres bloques llevan **`key` estable** o React **remonta** en vez de mover al cruzar 769 px. **El orden visual no cambia ni un pixel** (65 / 464 / 584). **(5) LA SUITE PASA DE 55 A 58, y dos flakies NO eran del producto**: corriendo la MISMA suite contra el `index.html` de HEAD, `checklist-foco` **falla igual**. Ese test pasa de `runFor(25 min)` —**1500 callbacks** con su re-render— a **`fastForward`**, que ademas prueba lo que antes nadie probaba: que el contador es **timestamp-based** (s96). Nace **`asentarGeometria()`** (espera a que `--pace-timer-d` repita valor tres frames), que **no** se mete dentro de `asentar` porque lo llaman veinte sitios, algunos con `page.clock`, y ahi rAF solo corre si el reloj avanza. **(6) EL INSTRUMENTO MINTIO CINCO VECES**: la pestaña del navegador embebido esta **`hidden`** y rAF no dispara (0 muestras en 9 s, con el `setTimeout` y el `MutationObserver` corriendo, o sea que parecia que medía) · **`[data-pace-dial-ring]` no existe en v0.89.0** (lo creo s158) y la primera pasada devolvio «sin arco» **justo para la mitad de la comparacion** · **headed no arranca aqui** (`spawn UNKNOWN`): todo medido en headless, el compositor real sin probar · mi sonda leia la variable **vacia** en las dos condiciones mientras una respondia y la otra no ⇒ se siguio la cadena eslabon a eslabon en vez de concluir · y **una edicion mia rompio el aro a mitad** (un `}` dejo el `height` colgando de los nodos interiores: aro a 327 px), cazado por la medida siguiente. **(7) VERIFICACION**: `npm run verify` **PASA** (7,6 s) con v0.91.0 coherente en los 3 sitios · **`npm run test:e2e` PASA 58/58**, dos pasadas seguidas · **banco de rojos 4/4**, cada uno con su mensaje y **no por un guard**, restaurando el artefacto byte a byte (`241DD403B5A1B84B`) en los cuatro. **(8) LO QUE NO SE CUBRE**: el tiron del arco **sigue abierto** a la espera del banco en su telefono · el **diagnostico de Respira (punto 4) NO se abrio** · ni un pixel comparado. Diario: [session-160](./docs/sessions/session-160-tiron-reduced-motion-y-foco.md).

**Sesion anterior:** #159 -- 2026-08-06 - **LA LUZ DEL POMODORO**. Bump **v0.89.0 -> v0.90.0**. **(1) SE PUBLICAN TRES SESIONES.** s157 y s158 nunca cerraron: su trabajo estaba en el arbol sin commit y `PLAN_ATMOSFERA_HOME.md` sin trackear, **ya desfasado** (describe defectos que s158 resolvio). **(2) EL CENTRADO MOVIL ERA PREVIO Y ESTABA PUBLICADO.** Medido sirviendo los dos artefactos en paralelo: la desviacion es IDENTICA en el arbol y en HEAD v0.89.0 — **+11,80 px a 320 · +11,89 a 360 · +12,00 a 375 · +12,09 a 390**. La atmosfera de s158 no lo introdujo: **lo hizo visible**, porque el halo cuelga del aro y arrastra el error a un campo de luz de 790 px. **La causa, recorriendo la cadena de ancestros**: el motor fija `--pace-dial-d` en **0,92 · W del VIEWPORT** y la raiz de `FocusTimer` añade ENCIMA `clamp(0px, 4vw, 40px)` por lado, que ese techo no descuenta; su max-content queda en **390,19 px** contra los **366** de ancho util, y `[data-pace-main-content]` es un **grid de UNA sola pista `auto`**: la pista crece, **DESBORDA y se coloca desde el START**. La mitad del desborde es la desviacion exacta. Arreglado con dos declaraciones en el bloque ≤768: anular ese padding (la causa) y `justify-content: center` (el guard) — a 320 la primera sola deja **2 px de holgura**, y el banco lo demuestra con un **control negativo** que deshace solo el padding y **sigue verde**. **0,00 px** en los cuatro anchos, sin tocar diametro, arco ni `home-geometry.js`. **(3) LA CURVA RESPIRA DESDE LA MITAD.** Las cuatro paradas de color no se tocan; lo que deja de ser lineal es el RECORRIDO. Y la meseta **sale de la curva**: `curvaSuave` tiene pendiente cero en sus extremos, asi que la subida se posa al llegar al pico en vez de doblar (antes hacia ESQUINA). La mitad que baja usa **otra** curva, `curvaCaida` (x^1,5): conserva la meseta pero **llega al final bajando**, que es lo unico que mata el repunte de p=1 — con la curva simetrica la luz se aplanaba en el minuto 25 mientras el color seguia subiendo. Medido en pixeles: pico de presencia **0,375 -> 0,495**, pico de calor OKLab **0,52 -> 0,495**, rebotes tras el pico **5 -> 2** y el de p=1 **eliminado**. Cola nocturna 0,46 -> **0,42**. **(4) EL PARPADEO, CON SU COSTE MEDIDO.** Dos cuantizadores sin suavizar: salto de color 4,9 % y de intensidad 4,7 %, **50 saltos por bloque, uno cada 30 s, 47 de ellos ≥2 %**. k 24->96 e i 30->120 lo dejan en **197 saltos (uno cada 7,6 s), mayor 2,03 %, UNO por encima del 2 %**. **No se añadio transicion de opacidad** y el usuario tenia razon al avisar: no suavizaria el escalon de COLOR, que vive dentro del degradado. Coste alternando tres modos en la misma pasada y separando trabajo real de protocolo: **+0,44 s** por bloque de 25 min (**0,029 %**), y la luz entera +0,61 s. **(5) LO QUE NO SE COMPENSA, POR DECISION EXPLICITA.** En la paleta CLARA la presencia compuesta culmina tarde (p≈0,77): sobre papel crema el atardecer contrasta mas que el mediodia. El **calor** si culmina centrado (0,495), que es lo que `tokens.css` ya declaraba. Medido, presentado y **no tocado**. **(6) TRES DEFECTOS QUE ENCONTRO EL USUARIO MIRANDO, Y LOS TRES ERAN TRANSICIONES QUE SE PISAN.** (a) La **sombra de los chips entraba a golpe**: el chip lleva `transition` de 0,22 s **INLINE sobre todas las propiedades** (su hover), asi que su `filter` perseguia con 220 ms de retraso a un `--pace-on` que viajaba 1,6 s — con la luz al 60 % la sombra iba por **0,01** de alfa y al pararse daba un tiron de **0,09 a 0,17** en 300 ms. No se puede ganar a un `transition` inline sin `!important`, y con el se quedaria sin hover ⇒ **la sombra se muda al GRID**, que no lleva transicion; misma forma proyectada, una declaracion en vez de cuatro. `--sun-cast` baja ademas de 0,18 a **0,10**. **Bajar el alfa sin lo otro habria tapado el sintoma dejando el corte.** (b) **Pausar era un salto de un frame** (0,517 -> 0,233): el 0,45 vivia dentro de `--pace-i`, que no se transiciona. Sale a un tercer mando, **`--pace-pausado`**, con fundido propio de 500 ms; y lo que publica el componente es el **interruptor**, no la profundidad — cuanto se recoge la luz es un valor **por paleta** (`--sun-pausa` 0,45 oscuro / 0,35 claro) y las paletas viven en CSS. (c) **La cola no llevaba el color del arco**: ahora `--pace-arco` publica el tono del recorrido y las dos paradas finales del bloom lo mezclan al 25 %. Aislado con todo lo demas congelado, el eje rojo-verde de la cola sigue al arco (**1,25 terracota -> 0,95 ocre -> 0,45 oliva**) y la presencia solo sube un 5-7 %: **tiñe, no ilumina**. **Solo la cola, no el limbo** — s158 bajo la saturacion un 10 % justo porque el halo llegaba a leerse como una ampliacion del arco. **(7) LA COLA, ELEGIDA MIRANDO.** Tres niveles comparados a tamaño real; el usuario eligio el intermedio (paradas 85 % y 92 % del bloom, 0,30/0,12 -> **0,50/0,28**), **sin tocar radio, caja ni anclaje**, que es lo que garantiza que no vuelva el scroll. **(8) DECIDIDO Y NO HECHO: el remapeo del arco.** El horizonte tapa **94,3°, el 26 % del bloque** con la cabeza invisible. Se monto un mock y el usuario lo aprobo **para su propia sesion**: toca el arco, los 60 ticks y `TimerDial`, compartido con Caminos. Sus cinco decisiones estan en el backlog. **El sidebar NO refleja**, decidido: es la superficie con mas texto pequeño de la app. **(9) DEUDA MECANICA**: `tests/home-geometria.spec.js` estaba en **631 lineas** (§1 fija 500) y se partio en cuatro **sin tocar una linea de cuerpo**. **(10) EL INSTRUMENTO MINTIO SIETE VECES, ninguna era el producto.** `getComputedStyle` de `--pace-luz` devuelve **`oklab(L a b / alfa)`**, no RGB, y el primer parser situo el maximo de calor en p=0 · la presencia medida como `max(0, L − L del papel)` valia **CERO** en las diez paradas —es un modelo de papel oscuro y la suite corre en claro— asi que el bucle de monotonia **comparaba ceros y pasaba con cualquier curva**; se arreglo con la **distancia OKLab completa** y su guard · apagar `--pace-on` **no es apagar la luz**: cierra ademas el horizonte del aro, y la diferencia incluia el arco (el pico saltaba de 41 a 121) · **`page.clock` congela el fundido de 1,6 s** y un guard lo cazo (halo 0,067); precision pagada dos veces: la transicion **si avanza en tiempo real**, lo que no hace es saltar con `fastForward` · `circles[ultimo]` del dial es el **PUNTO**, no el arco · el arco lleva **`transition: stroke-dashoffset 1s`** y capturar a los 200 ms fotografia la animacion a medio camino · **`spawnSync('npx.cmd')` sin shell no resuelve en Windows** y devolvio cuatro «rojos» con `exit=null` que no habian ejecutado un test (la trampa de s154 por la puerta contraria). Mas **backticks dentro del template literal de `_responsive.js` abortando el build DOS veces**, la misma de s139/s156/s157. **(11) VERIFICACION**: `npm run verify` **PASA** con v0.90.0 coherente en los 3 sitios · **`npm run test:e2e` PASA 55/55** contra el artefacto recien regenerado, las 44 previas sin una regresion · **banco de rojos: 5 asertos en rojo con mutacion controlada y restauracion por hash, 1 control negativo VERDE y 1 declarado sin rojo**; cuatro de los cinco **nacieron rojos contra el producto real** · navegador real en 5 combinaciones de viewport y paleta con sesion y pausa: **consola limpia**, desborde horizontal 0 · **cambio dia↔noche** a 25/50/75 % en los dos sentidos por la UI real con progreso y geometria **identicos** y sin fotograma de flash · `PACE_standalone.html` restaurado byte-identico. **(12) LO QUE NO SE CUBRE**: la deuda de **reduced-motion sigue intacta** (11 px a 1280×720, aro 420 vs 406) y es el siguiente trabajo tecnico pequeño; ni un pixel comparado en la suite; y el aserto del **alcance de la cola** no se ha conseguido poner rojo — se declara, con su razon medida. Diario: [session-159](./docs/sessions/session-159-luz-del-pomodoro.md).



**Ultima actualizacion de este archivo:** 2026-08-07 - sesion 160 (v0.91.0; se retiro del encabezado el resumen de la sesion #155, que sigue en `CHANGELOG.md` y en su diario — este archivo no debe crecer)
**Build entregado:** `index.html` **v0.91.0** (hash `241DD403B5A1B84B`; **no contiene ni un byte CR** (0 de 1 434 106): desde s153 el build normaliza los finales de linea al leer, asi que el artefacto **ya no depende del worktree de quien lo genera**. `PACE_standalone.html` restaurado **byte-identico** — hash `998e3e35...`, decision s134). **OJO**: verificar SIEMPRE el cierre cargando `index.html`, no solo `PACE.html` — el build envuelve cada modulo en un IIFE y hay fallos que solo salen ahi (el `useState` pelado de s144 estuvo DOS versiones publicado). **Desde s150 eso ya no depende de acordarse**: `npm run verify` lo caza por analisis de ambito y devuelve codigo de salida, y **desde s152 comprueba ademas catalogos, i18n, precache y glifos**. **Desde s154 hay ademas `npm run test:e2e`**, que abre un navegador de verdad sobre ese mismo `index.html` y ejecuta el checklist de cierre entero — es el **paso 4** del cierre y va DESPUES de regenerar, para probar el artefacto que se va a commitear; **s160 la deja en 58 tests (~60-80 s con 8 workers)** con los 2 del orden de foco y el de reduced-motion; **la prueba de Foco pasa de `runFor` a `fastForward`** porque 1500 callbacks con su re-render no caben en el plazo bajo carga — y de paso aserta que el contador es timestamp-based. **El CI repite los tres en cada push**. `PACE_standalone.html` sigue en v0.71.0 A PROPOSITO (export bajo demanda, s134).
...
- ~~**[REDUCED-MOTION — deuda de s156]**~~ **CERRADA en s160/v0.91.0.** La microcausa era que el
...
## Proxima sesion -- **EL CI SIGUE ROJO** (espera `gh auth login`), luego DIAGNOSTICO DE RESPIRA, luego el tiron del arco

> **LO PRIMERO ES EL CI**, y es lo unico que no puede esperar otra sesion: los runs **#7
> (v0.90.0) y #8 (v0.91.0) estan ROJOS** y s159 y s160 cerraron declarando verde — lo verde era
> **local**. s161 localizo el rojo sin adivinar (el repositorio es publico y la API de *jobs*
> responde sin auth): **`verify` VERDE y `e2e` ROJO** en los dos. Descartado midiendo que sean
> los 2 workers, las mayusculas de rutas ni el lockfile ⇒ **es Linux**. Los *logs* dan **403 sin
> auth** y `gh` ya esta instalado (2.97.0), asi que **falta solo `gh auth login`**, que es accion
> del usuario. Con eso, `gh run view --log-failed` lo cierra en un comando.
>
...
> respiracion, ronda y total pueden estar diciendo lo mismo tres veces) y traer un menu de
> decisiones, **sin implementar nada**.
>
> **El tiron del arco sigue esperando** el banco de cuatro aros en el telefono del usuario (ver
> «El tiron del arco» en Pendiente): sin ese dato no se toca el Pomodoro.
>
> **La red de seguridad esta COMPLETA en sus dos mitades**: la ESTATICA (`npm run verify`, 4
> tandas, ~8 s) y la de COMPORTAMIENTO (`npm run test:e2e`, **65 tests, ~80 s**). **El CI repite
