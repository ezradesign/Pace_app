...
**Ultima actualizacion de este archivo:** 2026-08-17 - sesion 162 (v0.93.0; se retiro del encabezado el resumen de la sesion #159, que sigue en `CHANGELOG.md` y en su diario - este archivo no debe crecer)
**Build entregado:** `index.html` **v0.93.0** (hash `F4DDEE6F229DD346`; **no contiene ni un byte CR** (0 de 1 434 106): desde s153 el build normaliza los finales de linea al leer, asi que el artefacto **ya no depende del worktree de quien lo genera**. `PACE_standalone.html` restaurado **byte-identico** — hash `998e3e35...`, decision s134). **OJO**: verificar SIEMPRE el cierre cargando `index.html`, no solo `PACE.html` — el build envuelve cada modulo en un IIFE y hay fallos que solo salen ahi (el `useState` pelado de s144 estuvo DOS versiones publicado). **Desde s150 eso ya no depende de acordarse**: `npm run verify` lo caza por analisis de ambito y devuelve codigo de salida, y **desde s152 comprueba ademas catalogos, i18n, precache y glifos**. **Desde s154 hay ademas `npm run test:e2e`**, que abre un navegador de verdad sobre ese mismo `index.html` y ejecuta el checklist de cierre entero — es el **paso 4** del cierre y va DESPUES de regenerar, para probar el artefacto que se va a commitear; **s162 la deja en 67 tests (~50 s con 8 workers)** con los 2 del orden de foco y el de reduced-motion; **la prueba de Foco pasa de `runFor` a `fastForward`** porque 1500 callbacks con su re-render no caben en el plazo bajo carga — y de paso aserta que el contador es timestamp-based. **El CI repite los tres en cada push**. `PACE_standalone.html` sigue en v0.71.0 A PROPOSITO (export bajo demanda, s134).
...
## Proxima sesion -- **TROCEAR LOS CINCO DE LA REGLA 1**, luego DIAGNOSTICO DE RESPIRA, luego el tiron del arco
...
>
> **LO PRIMERO ES LA REGLA 1**, que desde s162 la vigila el `verify` con un trinquete:
> **cinco archivos siguen por encima de 500 lineas** y estan congelados en
> `scripts/verify.tamano.js` (`DEUDA_500`). Ninguno puede crecer y ninguno nuevo puede
> aparecer, pero la deuda **sigue ahi**: `_responsive.js` **1132** · `FocusTimer.jsx` 686 ·
> `tokens.css` 676 · `TweaksPanel.jsx` 534 · `state-core.jsx` 515. Al trocear cada uno hay
> que **borrar su fila** de `DEUDA_500` (el propio verify lo exige). **Tres de los cinco son
> el sistema visual de la home y la suite no compara ni un pixel**: ese troceo pide una
> pasada VISUAL del usuario al terminar, que es el detector que s147 declaro para lo visual.
> Los dos de estado (`state-core.jsx` 15 de mas, `TweaksPanel.jsx` 34) son los baratos y
> tienen su patron hecho (`*.support` / `*.parts`).
>
...
> - **Proteger `main`** — `WORKFLOW.md` §8, **accion del usuario**. `gh` ya esta instalado y
>   autenticado (s161), asi que la afirmacion de la auditoria **ya se puede verificar**. Y ojo con la trampa
...
- ~~**Arreglo sin decision, aparcado para no mezclar frentes**~~ **YA NO APLICA (s162)**: la
  cabecera de `home-geometry.js` dice hoy «UN MOTOR, DOS PIELES (corregido en s149)» y «el motor
  corre en TODO viewport y publica las dos variables siempre». Texto original: `home-geometry.js:26` dice que el
