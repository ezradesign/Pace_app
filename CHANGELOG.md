...
## [v0.117.1] -- 2026-09-06 -- fix(respira): la tarjeta de Viajes deja de ser negra

### Cambiado
- **El estante ya no es un bloque negro.** La tarjeta de un viaje lleva ahora un **lavado del color
  de Respira** sobre el papel de la tarjeta y **los pulmones del modulo, grandes, sangrando por el
  borde derecho**. Se eligio mirandolo: 19 variantes pintadas sobre la app de verdad -- terracotas,
  tres olivas, tabaco, pizarra calida, el azul de Hidratate, lavados, y el dibujo en seis tamanos y
  posiciones.
- **El porque, en una frase**: el negro distinguia pero no decia de quien era el viaje. El dibujo
  dice las dos cosas -- que es otra cosa y que es Respira- sin gastar una palabra.
- **El terracota puro nunca fue viable**: sobre crema da 2,80:1 y el cuerpo de la tarjeta es de
  13 px, o sea texto pequeno. Solo funciona con el texto en tinta oscura (5,04:1) o bajando el tono.
- **Un argumento de SISTEMA descarto los verdes**, por bonitos que fueran: en PACE cada modulo tiene
  su color -- verde es Foco, tabaco es Mueve, azul es Hidratate-, asi que un fondo verde dentro de la
  biblioteca de Respira dice «esto es de Foco» a quien ya aprendio el codigo.
- **El lavado se compone con TOKENS**: `--breathe-soft` sobre `--paper-2`. Con un hex a fuego, en la
  paleta oscura habria quedado un bloque claro deslumbrando; asi el fondo real en oscuro sale en
  rgb(38,33,26) sin una sola regla de tema. La marca es `--breathe-2` al 22 %.
- **El dibujo va DETRAS del texto y es decorativo** (`aria-hidden`): el nombre y la duracion ya lo
  dicen todo, y un dibujo que se anuncia en el arbol de accesibilidad solo mete ruido.

### Anadido -- red de seguridad
- **3 tests mas**: que la tarjeta lleva la marca, que va detras del texto y muda para un lector de
  pantalla, y —**uno por paleta**— que el cuerpo y la linea de datos **se leen sobre el lavado**,
  medido EN LA PAGINA con los colores computados. Su mutante, subir el lavado a `--breathe` entero,
  pone rojas las dos paletas.

### La trampa de siempre, otra vez
- **Un backtick en un comentario de `library.css.jsx` aborto el build.** Ese archivo declara su CSS
  dentro de un template literal y su propia ficha lo avisa; van tres veces. El comentario nuevo lo
  dice otra vez, ahora desde dentro.

---

## [v0.117.0] -- 2026-09-06 -- feat(respira): el estante de Viajes

### Anadido
- **La biblioteca de Respira tiene un segundo estante: «Viajes».** Es la separacion que pide la
  Fase 5 del ROADMAP, y la diferencia se lee antes que el texto: tarjeta en oscuro, nombre y
  cifra grandes, arriba del todo. Dice «esto es otra cosa» para que nadie abra veinticinco
  minutos creyendo que abre cuatro.
- **Los viajes llegan por su propia lista y su propia prop**, nunca por `BREATHE_ROUTINES`. Las
  tres reglas que necesitan salen de esa estructura y no de tres excepciones: no los cuenta el
  filtro, no los propone «Para ahora» y no compiten en el orden de los grupos.
- **`window.BREATHE_VIAJES` esta vacia a proposito.** El contenido CTB esta FUERA de la v1
  (decision del usuario, s180); lo que entra en v1 es el sitio. Sin viajes no se pinta nada, ni
  la cabecera -- la misma regla que ya gobierna los grupos vacios.
- **La linea de un viaje dice lo suyo**: cuantos TRAMOS tiene y que lleva musica. No dice su
  ritmo (cambia cinco veces) ni de que posicion es.

### Corregido, antes de que pudiera pasar
- **Un viaje `access: 'premium'` se habria abierto GRATIS.** `canAccessRoutine` resuelve el id con
  `getBreatheRoutine` y es **fail-open** con los ids que no conoce -- por diseno, y con razon-, asi
  que un viaje no habria estado en ningun catalogo y el guard habria dicho «adelante».
  `getBreatheRoutine` busca ahora tambien en la lista de viajes: un viaje es una rutina de Respira
  a todos los efectos de acceso y de seguridad, y lo unico que cambia es donde se pinta. Con su
  mutante en rojo.

### Anadido -- red de seguridad
- **6 tests** en `tests/viajes-estante.spec.js`: que sin viajes no se pinta NADA, que con uno va
  arriba y con otra tarjeta, que los chips no lo cuentan, que «Para ahora» no lo propone, que un
  viaje con retencion pasa por el MISMO modal de seguridad, y que un viaje premium esta cerrado.
- **Un mutante declarado que NO muerde, con su razon**: meter los viajes en `todas` no cambia nada
  en pantalla porque ese `useMemo` depende de `[groups]` y se calcula al arrancar la app, cuando
  `window.BREATHE_VIAJES` todavia esta vacia. Los asertos 3 y 4 son GUARDIAS de un refactor
  futuro, no pruebas -- y no cazarian a alguien que escribiera un viaje directamente en
  `BREATHE_ROUTINES`, porque ese catalogo es un `const` que no se alcanza desde la pagina. Lo que
  sostiene la regla es la estructura.

### Lo que NO cubre, y se declara
- **No hay runner de viajes.** El guion y la pantalla estan en `docs/proposals/ctb-marea-baja.html`
  y nada mas; por eso el aserto de la puerta llega hasta el modal de seguridad y ahi se para.
- El estante **no se prueba en ingles**, solo el camino en espanol.

---

## [v0.116.0] -- 2026-09-06 -- feat(respira): la sesion que se puede retomar

### Anadido
- **Una sesion de Respira interrumpida se puede REANUDAR.** Clave `pace.breathe.v1`, fuera de
  `pace.state.v2`, el mismo patron que el Pomodoro (s102). La sidebar la ofrece con la ronda:
  «CONTINUA · Rondas express · Ronda 2 de 2».
- **Lo que NO se guarda es la decision**: ni la fase ni el segundo dentro del ciclo. Nadie se
  reengancha a mitad de una inhalacion que no estaba haciendo; devolverte al segundo 3 de una
  exhalacion seria fingir una continuidad que tu cuerpo no tuvo. Se guarda la RONDA (y en las
  rutinas sin rondas, el tiempo practicado), y al volver se entra otra vez por la cuenta atras.
- **No se acredita nada no presenciado** (linea s101/s102): el reloj de tiempo activo solo corre
  en 'active'/'hold' sin pausar, asi que el rato fuera no suma; al volver CONTINUA en vez de
  reiniciarse -- ni se regalan minutos ni se pierden los practicados.
- **Caduca a las 2 h y dentro del mismo dia.** Es un juicio declarado, no una medida: una sesion
  de respiracion es un estado en el que estabas, no una tarea pendiente.
- **Se conserva al SALIR**, y ahi esta la diferencia con el Pomodoro: salir de una sesion de
  Respira ES la interrupcion que se quiere recuperar. Solo se borra al terminar.
- **Reanudar entra por `handleStartBreathe`**, la misma puerta que empezar, asi que una rutina
  con apnea vuelve a pedir su modal de seguridad. Hay un mutante que lo defiende.
- **`docs/proposals/ctb-marea-baja.html`**: el prototipo de CTB (guion de 25 min + tres pantallas
  a 390x844 + donde vive). Elegida la variante **B**.
- **`scripts/audit/censo-adaptacion-s186.js`**: cuenta si el catalogo puede sostener «cambia el
  ejercicio que no puedas hacer». No puede todavia, y por eso existe el censo.

### Cambiado
- **`useActiveClock` admite una semilla** para que un reloj pueda CONTINUAR otro. Sin argumento
  se comporta igual que antes.
- **`PHASE_KEYS` se muda a `BreatheSession.support.jsx`** por la regla §1: el runner llego a 500
  lineas exactas al entrar la reanudacion. Es dato, no logica, y ese archivo nacio para esto.
- **`Sidebar.selectors.js` deja de describir una deuda que ya no existe.** La nota decia que el
  runner no persistia ronda ni fase; ahora la persiste. La que sigue pendiente es «CTB
  recuperable», y sigue pendiente porque no hay sesiones CTB.

### Medido, y cambia lo que se iba a construir
- **31 rutinas declaran los cinco metadatos de adaptacion al 100 % · 0 de 129 EJERCICIOS.** La
  posicion, el equipo y la intensidad las declara la RUTINA, no el ejercicio, asi que ofrecer un
  sustituto con criterio exige anotar 129 ejercicios: **contenido, no codigo**. Saltar el
  ejercicio ya se puede hoy.
- **19 pasos de descanso, y solo 6 son `restKind: 'betweenSets'`.** «Alargar el descanso» se
  aplicaria a seis sitios; los otros 13 son cierres respiratorios que s114 dice no tocar.
- **El recordatorio de agua tiene el dato y no la entrega.** `water.goal`, `water.today` y
  `waterGoalDates` existen, pero una PWA no dispara un aviso con la app cerrada sin servidor de
  push, y este producto es offline-first con licencia offline. En web solo cabe un aviso in-app;
  el de verdad llega con Capacitor (Fase 9). Ademas `state.reminders: []` es un campo MUERTO
  desde v0.11.6.
- **Movil, la revision que s185 declaro sin hacer**: sin regresion. Fila de minutos 0-2,7 sobre
  255 · halo pegado al aro identico al de escritorio · cero scroll. Y el numero quedo **mejor por
  accidente** (29/24,5 donde antes ~38/16), porque la compensacion del subtitulo es solo de
  escritorio: se dice tal cual, no fue una decision.

### Lo que NO cubre, y se declara
- Los cinco tests de reanudacion corren a tamano de escritorio; el camino de movil no se prueba.
- **El estante «Viajes» esta decidido y pintado, no construido.**
- La reanudacion es solo de Respira: Mueve y Estira tienen su propio runner.

---

## [v0.115.1] -- 2026-09-05 -- fix(home)+docs(musica): tres premisas caducadas

> Sesion de REVISION. No hubo encargo nuevo: el usuario probo v0.115.0 y fue senalando. Cada cosa
> que vio resulto cierta y medible **despues** de verla, y en tres casos la causa era **una premisa
> escrita al lado del codigo que gobierna, que habia dejado de ser verdad**.

### Corregido
- **La bola guia se metia 1,2 px dentro de la tarjeta.** El aire eran 6 px FIJOS y el halo de la
  bola mide **7,1 px de radio** (0,017 D). Pasa a **0,030 D**, proporcional.
- **El halo, arriba.** Tres intentos y los dos primeros eran el mismo problema: rampa recta → CODO
  visible · S larga → apaga el NUCLEO (0,505-0,528 D, pegado al trazo) · S corta → canto plano sobre
  un halo redondo. **La corona no cabia**: muere a 0,628 D y sobre el aro habia 24,5 px, no los ~59
  que afirmaba el comentario. Se le da sitio con `margin-top: 0,055 D` en el `timerWrap` -- y se
  puede porque a 1280x800 el aro esta topado por ANCHO y a 1920x1080 por el tope de 520, no por
  altura. Hueco aro←fila: **24,5 → 47,6 px**.
- **El numero no estaba donde decian las cajas.** Por caja, 10,9 px de «FOCO MANUAL» y 32,3 de «Foco
  breve»; medido en **TINTA**, **53 y 23** -- lo contrario. El `line-height: 0.9` de un serif de
  display deja ~42 px de aire muerto sobre los glifos. Y un margen negativo en el numero **se lleva
  todo lo de abajo**, asi que hay que restar arriba y sumar lo mismo abajo. Queda en **43/36**.
- **La home hacia scroll vertical en escritorio**: 29 px a 1536x864 y 11 a 1600x900 (15 ya en lo
  publicado). La causa es la caja del BLOOM. La premisa decia «el hueco entre el centro del aro y el
  borde es 0,96 D en el peor breakpoint» y se tomo en cuatro breakpoints **sin el peor**: el real es
  **0,852 D**. La caja pedia 0,909 → no cabia por 30 px, y el scroll medido eran 29; a 1600x900
  predice 11 y mide 11. `BLOOM_H` 1,42→1,34 · `BLOOM_SUBE` 0,36→0,38 · `BLOOM_R` 0,84→0,77. Coste
  medido: luz lateral 72→67, en Actividades 93→85.
- **El margen del halo se volvia transicion con reduced-motion** y el aro convergia distinto (381
  contra 379). `[data-pace-timer-wrap]` entra en la exencion de s160/s162.

### Cambiado -- documentacion
- **`MUSICA_RESPIRA_BRIEFS.md` deja de pedir dos cosas incompatibles.** Los briefs exigian «rango
  medio despejado (200 Hz-3 kHz)» y s177 exige el contrario porque MIDIO que esa restriccion hacia
  la pieza inaudible. **Lo que destraba la contradiccion es que el brief describia mal a su
  competidor**: la senal sintetizada no es un tono sino RUIDO con paso-bajo barriendo 200→800 Hz,
  `breathe.hold` no existe, y **con musica esa senal no suena**. Solo compite la locucion, y el
  **35,9 % del ciclo** de media (15 % en Kumbhaka, 72 % en Rondas). La separacion pasa a ser por
  COMPORTAMIENTO -- la voz se mueve sobre un fondo plano-- y por el tramo alto (poco sobre 2 kHz,
  donde viven las consonantes). **Segundo conflicto, que la auditoria no habia visto**: cinco
  prompts pedian «a sustained low G» y el de Relajacion «everything lives in the low and low-mid
  register», o sea exactamente la pieza que s177 midio inaudible. La raiz puede ser grave; el TIMBRE
  no puede ser un pad casi senoidal. **Ya se puede generar.**

### Anadido -- red de seguridad
- **4 tests «la home de escritorio no hace scroll vertical»**, en los dos estados y en los cuatro
  viewports criticos. Miden moviendo `scrollTop`, **no** restando `scrollHeight`: la resta da falsos
  positivos con cualquier capa absoluta, que es justo por lo que el motor dejo de usarla. **No lo
  vigilaba nadie**: el motor mide el stack (s156) y el stack cabia; la suite comparaba luz encendida
  contra apagada y el desborde era **identico en los dos estados**.
- **El aire del cabo pasa a ser RELACIONAL al halo de la bola**, no un numero escrito: el aserto de
  «recorte y solapamiento» exige que quepan los 0,017 D del halo y que el cabo no se aleje mas de
  0,05 D. Vive en `home-geometria.spec.js`, junto a lo que ya media ese corte.
- **Un mutante de temporizacion se calibra con el SPEC ENTERO, no con `-g`**: el del margen del halo
  fuera de las exenciones pasa 6 de 6 corriendo su test solo, y muerde a la primera con el archivo
  entero en paralelo. En aislamiento el frame tardio llega a tiempo igual -- la misma condicion que
  s162 llamo «rojo intermitente».
- **`scripts/audit/revision-aro-s185.js`**: la maqueta de los nueve viewports de escritorio con sus
  numeros debajo. **Volvio a recortar en su primera version** -- scroll por figura no es enseñar la
  captura-- que es literalmente el defecto de s182; corregido y anotado dentro.

### Lo que NO cubre, y se declara
- **A 1366x610 el aro esta limitado por ALTURA**, se come el margen del halo y quedan 5,3 px sobre
  ACTIVIDADES. Reservarle holgura al motor seria meterse en el bucle de «encoger hasta caber».
- **Que un degradado tenga un codo o un canto plano no lo caza ningun aserto** -- es criterio, y lo
  detecta la revision a tamano real. Los tres intentos del halo los distinguio el usuario mirando.
- **La regla de no-scroll solo se vigila en escritorio**, que es donde se pidio. En movil el scroll
  es legitimo, y los numeros de movil salen del banco, no de la maqueta de esta sesion.
