...
Simulado creciendo la fila y midiendo qué hace el aro:
...
**Fila propia, gateada por las dos dimensiones y no por la altura sola.** La
condición honesta es «hay 42 px que no le quita al aro», y eso ya lo sabe el
motor: `--pace-home-squeeze` vale 0 mientras no haya que comprimir nada. Una
regla del tipo «mostrar solo con `squeeze == 0` y alto ≥ 736» cubre 375 y 414 sin
castigar a 320, y **no elige un número a ojo en una sola altura**.
...
Y una que nace aquí: **la caché de Chromium del CI entró sin verificar**. Un
cambio de YAML solo se prueba empujando, así que el primer push debe mirar el
paso «Cache de Chromium» y confirmar que acierta.
