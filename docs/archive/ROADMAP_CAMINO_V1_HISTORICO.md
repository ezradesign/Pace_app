# ROADMAP — seccion «Camino a v1.0» ANTERIOR (archivada)

> **HISTORIA. No gobierna.** Secuencia adoptada en s93 y sustituida en s132 por el plan
> operativo unico de `ROADMAP.md`. Llevaba 23 sesiones mostrando como «siguiente» la fila
> `s107 · Caminos al centro`, que nunca se ejecuto, mientras las sesiones reales seguian
> otro orden (B1, B2 y Bloque 0). Se conserva por su registro de lo YA HECHO (s94-s121) y
> por los backlogs que menciona.

> Indice de autoridad: [`docs/product/AUDITORIA_DOCUMENTAL.md`](../product/AUDITORIA_DOCUMENTAL.md).
...
| ~~B1.2~~ | **hecho (v0.53.0, sesión #108 — CIERRA B1)** — editorial de seguridad ES+EN completo con criterio realista-explicativo + BASE §7-9 (al fallo/al límite/al máximo/aguanta secos fuera · «hombro nace para colgar» e «indestructibles» fuera · chin tuck sin «papada» · PULL→PUSH · 12 descs anuncian suelo/pared/barra firme/silla estable sin ruedas · **Dead hang · opcional** con alternativa, key de glifo renombrada en sincronía) + curación Respira·Energía (Bhastrika → grupo Pranayama; `rounds.express` FREE) + **defaults opt-out** (`soundOn:true` + `notifyFocusEnd:true` en instalaciones nuevas; permiso de notificación en el primer «Comenzar» de Foco, denegar apaga el flag) |
| **B2** | **Fundamentos** (en curso; detalle canónico en [`DECISIONES_PRODUCTO.md`](./docs/product/DECISIONES_PRODUCTO.md) §B2): ~~auditoría s109~~ · ~~visualId+contrato v1 pilotado s110~~ · ~~método s111~~ · ~~afinado UX s112~~ · **GIRO runner guiado (2026-07-18, enmienda R2/R3/BASE §3-A) — CERRADO 2026-07-21**: ~~s113 motor~~ (v0.57.0: reps guiadas con cadencia + transición auto de lado + rest tipado + layout por altura) · ~~s114 capa editorial~~ (v0.58.0: instrucciones por capas placeCue/shortCue/careCue + pantalla final por módulo con reps guiadas reales —resuelve el P3 antidoteDone— + Tweaks «Sesiones» descanso 20/30/45 solo betweenSets + audio sin voz move.warn/move.side) · ~~B2.2b-1 contrato+duración derivada s115~~ (v0.59.0: `instruction.*` + `tempo` + `transition` + `completion` + los dos `setup` (mode/copy) + metadatos + `estimateDuration` PURO + fix peso=progreso=dur; migración atómica, legacy byte-idéntico) · ~~B2.2b-2 feedback ligero «¿te ayudó?» s116~~ (v0.60.0: bloque en el DONE fuera de Caminos + slice `routineFeedback{yes,some,no,lastPromptDay}` conteos completos —`answered`/`helpScore` derivados— + frecuencia 1×/rutina/día + guard de teclado del done; SOLO captura, sin consumidor visible) · ~~B2.2b-3 eventos solo diseño s117~~ (docs-only, sin bump: [`EVENTOS_SCHEMA.md`](./docs/product/EVENTOS_SCHEMA.md) rev.5 — **arquitectura por adaptadores**, P0 single-writer resuelto, **Android e iOS Capacitor como runtimes previstos**; **DISEÑO aprobado, NO implementado**) · ~~B2.3 OLA 1 s118~~ (v0.61.0: 5 rutinas Mueve gratuitas —chair.dips·calves·grip.squeeze·glutes.stealth·posture.set— al contrato v1 + `couch.stretch.min` 5→6; migración MECÁNICA en olas de 5–6, reescrituras APARTE en ola editorial, decisión s118; conteo real 23 legacy→18) · ~~s119 FASE A (estabilidad de layout del runner v1: barra fantasma + anclaje del glifo + warning rep-pulse, delta 0 de comportamiento) + B2.3 OLA 2~~ (v0.62.0: 5 rutinas Estira —wrists·shoulders.5·shoulder.circles·hips.5·morning.flow— al contrato v1; movilidad→timed, estiramiento bilateral→perSide+transición, fuerza→reps, flujo→timed+rest; gate `ready` en suelo/pared/barra; 18→13 legacy) · ~~B2.3 OLA 3 s120~~ (v0.63.0: 5 rutinas mixtas —hang.bar·core.stealth·back.desk·spine.waves·hamstrings— al contrato v1; holds isométricos→timed con **rests suaves sin restKind**, Superman/Scapular squeeze→reps con **Superman `reps`+`ready` sobre suelo**, Isquio a una pierna→perSide; **SIN intercambio de acceso** —la cifra «1 free + 6 premium» del corte describía solo el subconjunto legacy, no el catálogo; decisión del usuario, entitlement intacto; 13→8 legacy) · ~~B2.3 OLA 4 s121~~ (v0.64.0: `core.plank`+`wall.sit`, las 2 ÚNICAS rutinas mecánicamente tractables restantes; aguantes isométricos→timed con care rodillas/altura DERIVADA, Plancha lateral «30s/lado»→perSide `dur:30` POR LADO=legacy, rests suaves sin restKind, gate `ready` suelo/pared, **wall.sit conserva 60s** —disciplina de dosis s120—, acceso INTACTO ambas premium; **migración MECÁNICA de B2.3 CERRADA**, 8→6 legacy) · **6 rutinas legacy restantes BLOQUEADAS por reescritura editorial / progresión técnica / revisión fisio (`atg.knees` espera revisión FISIO de Sissy squat); NO son deuda mecánica** · ~~s122 CLARIDAD UX de la home~~ (v0.65.0: sistema verbal «Empezar foco»/«Iniciar camino»/«Ver caminos» + «FOCO MANUAL» dentro del círculo + tarjeta de Camino que se explica sola —eyebrow+duración+iconos+CTA— + jerarquía §1 Camino↑Actividades + solapamiento editorial «sol» con `transform` gate ≥760px; **§0 short-viewport y §7 diferidos**; **hallazgo diferido**: scrollbar del runner v1 a ≤660px en pasos perSide de texto largo) · **PRÓXIMA CONDICIONADA a la prueba real de la home** (ajuste corto de claridad si hay bloqueo · sesión de runner responsive · §0 completo · ola editorial s123). **Base de conocimiento offline**: [`docs/product/BASE_MUEVE_ESTIRA.md`](./docs/product/BASE_MUEVE_ESTIRA.md) (NHS/ACSM/OMS → reglas de unidad por tipo de ejercicio, duración calculada, auditoría ejercicio-a-ejercicio ANTES de tocar código) |
...
| post-venta | Vite/ESM real (Etapa B) · Path Builder (reuso F7) · Modo Estudio · sonidos procedurales premium · CTB completa · **Android + iOS via Capacitor** (notificaciones/hápticos/widget/billing; iOS "cuando corresponda" — feedback s102-cierre; **arquitectura de persistencia de eventos por adaptadores APROBADA en s117** — [`EVENTOS_SCHEMA.md`](./docs/product/EVENTOS_SCHEMA.md) §19: SQLite nativo + Preferences/UserDefaults, transacciones nativas; **implementación pendiente**) · Wrapped · extensión Chrome · IndexedDB via localForage (antes: `navigator.storage.persist()` tras onboarding, barato) |
